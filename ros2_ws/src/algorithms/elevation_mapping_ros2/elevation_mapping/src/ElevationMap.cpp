...
ElevationMap::ElevationMap(rclcpp::Node::SharedPtr nodeHandle)
    : nodeHandle_(nodeHandle),
      ...
      postprocessorPool_(ElevationMap::getPostprocessorThreads(nodeHandle),
                         nodeHandle),
  ...
  if (!underlyingMapTopic_.empty()) {
    underlyingMapSubscriber_ = nodeHandle_->create_subscription<grid_map_msgs::msg::GridMap>(
      underlyingMapTopic_, 1,
      [this](grid_map_msgs::msg::GridMap::SharedPtr msg) { ElevationMap::underlyingMapCallback(msg); }
    );
  }
  ...
  initialTime_ = nodeHandle_->now();
}

ElevationMap::~ElevationMap() = default;

std::size_t ElevationMap::getPostprocessorThreads(rclcpp::Node::SharedPtr nodeHandle) {
  std::size_t result;
  nodeHandle->declare_parameter("postprocessor_num_threads", 1);
  nodeHandle->get_parameter("postprocessor_num_threads", result);
  return result;
}
...
void ElevationMap::underlyingMapCallback(const grid_map_msgs::msg::GridMap::ConstPtr& underlyingMap) {
  ...
  if (underlyingMap_.getFrameId() != rawMap_.getFrameId()) {
    RCLCPP_ERROR_STREAM(nodeHandle_->get_logger(),
                        "The underlying map does not have the same map frame ('" << underlyingMap_.getFrameId() << "') as the elevation map ('"
                                                                              << rawMap_.getFrameId() << "').");
    return;
  }
...
bool ElevationMap::add(const PointCloudType::Ptr pointCloud, Eigen::VectorXf& pointCloudVariances, const rclcpp::Time& timestamp,
  ...
  if (static_cast<unsigned int>(pointCloud->size()) != static_cast<unsigned int>(pointCloudVariances.size())) {
    RCLCPP_ERROR(nodeHandle_->get_logger(),
                 "ElevationMap::add: Size of point cloud (%i) and variances (%i) do not agree.", (int)pointCloud->size(),
              (int)pointCloudVariances.size());
    return false;
  }

  // Initialization for time calculation.
  const rclcpp::Time methodStartTime(nodeHandle_->now());
  const rclcpp::Time currentTime(nodeHandle_->now());
  ...
  const rclcpp::Duration duration = nodeHandle_->now() - methodStartTime;
  RCLCPP_DEBUG(nodeHandle_->get_logger(), "Raw map has been updated with a new point cloud in %f s.", duration.seconds());
  return true;
}
...
bool ElevationMap::fuse(const grid_map::Index& topLeftIndex, const grid_map::Index& size) {
  ...
  const rclcpp::Duration duration(nodeHandle_->now() - methodStartTime);
  RCLCPP_DEBUG(nodeHandle_->get_logger(), "Elevation map has been fused in %f s.", duration.seconds());
...
bool ElevationMap::fuseArea(const Eigen::Vector2d& position, const Eigen::Array2d& length) {
  RCLCPP_DEBUG(nodeHandle_->get_logger(), "Requested to fuse an area of the elevation map with center at (%f, %f) and side lengths (%f, %f)", 
...
bool ElevationMap::publishVisibilityCleanupMap() {
  ...
  grid_map_msgs::msg::GridMap::SharedPtr message = grid_map::GridMapRosConverter::toMessage(visibilityCleanupMapCopy);
...
void ElevationMap::visibilityCleanup(const rclcpp::Time& updatedTime) {
  ...
  rclcpp::Duration duration(nodeHandle_->now() - methodStartTime);
...
bool ElevationMap::publishFusedElevationMap() {
  ...
  grid_map_msgs::msg::GridMap::SharedPtr message = grid_map::GridMapRosConverter::toMessage(fusedMapCopy);
